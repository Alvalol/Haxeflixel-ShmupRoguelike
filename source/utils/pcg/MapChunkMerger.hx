...
class MapChunkMerger
{
	...
	private static var CHUNKS:Array<TiledMap> = []; // chunks as TiledMaps for use in LEVEL.
	public static var CHUNKDATA:Array<Array<Int>>;
	...
	public static function makeCleanArray():Array<Array<Int>>
	{
		...
		for (chunk in CHUNKS)
		{
			...
			var layerData = layer.tileArray;
			var formattedChunk = formatArray(layerData);
			var chunkWidth = layer.width;
			var chunkHeight = layer.height;
			CHUNKDATA.push(formattedChunk,chunkWidth,chunkHeight);
		}
		...
		private static function formatArray(chunk:Array<Array<Int>>,chunkWidth:Int,chunkHeight:Int):Array<Array<Int>>
	{
		...
		for (i in 0...chunk.length +1)
		{
			...
			{
				newArray.push([]);
				for (j in i...(i+chunkWidth))
				{
					newArray[Std.int(i / chunkWidth )].push(chunk[j]);
				}
			}
		}
